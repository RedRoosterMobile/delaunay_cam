...
#define SCREEN_SIZE "notest" // "test" for small
#define TARGET_FRAME_TOLERANCE 3
#define LINE_HEIGHT 15
#define FONT_SIZE 10
#define RANDOM_POINTS 800 // 800 OK with fps on 2,3 GHz Intel Core i7
...
//--------------------------------------------------------------
void ofApp::draw(){
    ofSetWindowTitle(ofToString(TARGET_FRAME_RATE));
    // ofFill();
    // black
    //1440 x 900
    if (fullScreen) {
        float fullWidth  = ofGetViewportWidth();
        float fullHeight = ofGetViewportHeight();
        ofScale(fullWidth/w,fullHeight/h,1);
    }
    ...
    // mesh.draw(0,0, ofGetWidth(), ofGetHeight());
